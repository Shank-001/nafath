  ...
  class NafathRequestsController < ApplicationController
    ...
    def receive_callback
      ...
      unless matching_personal_info
        return render json: { error: "No matching Personal Information record found
                                      for the provided request ID and transaction ID." },
                      status: :not_found
      end
      ...
      if user_info["status"] == "COMPLETED"
        ...
        logger.info("User authentication completed for request ID and trans ID:
                      RID:#{request_id} TID:#{trans_id}")
    ...
    def store_request_id_and_trans_id(request_id, trans_id)
      ...
      unless latest_personal_info
        raise ActiveRecord::RecordNotFound,
              "Unable to store request ID and transaction ID.
              No Personal Information record was found for the current user."
      end
